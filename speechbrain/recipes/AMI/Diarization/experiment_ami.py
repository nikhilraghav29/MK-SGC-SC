...
def compute_embeddings_old(wavs, lens):
...
def diarize_dataset(full_meta, split_type, n_lambdas, pval, n_neighbors=10):
    ...
    for rec_id in tqdm(all_rec_ids):
        ...
        # File to store embeddings.
        if split_type == "eval":
            emb_file_name = rec_id + "." + params["mic_type_Eval"] + ".emb_stat.pkl"
            diary_stat_emb_file = os.path.join(
                params["embedding_dir"], split, emb_file_name
            )
            ...
            # Prepare a metadata (json) for one recording. This is basically a subset of full_meta.
            # Lets keep this meta-info in embedding directory itself.
            json_file_name = rec_id + "." + params["mic_type_Eval"] + ".json"
            meta_per_rec_file = os.path.join(
                params["embedding_dir"], split, json_file_name
            )
        ...
        # Prepare data loader.
        diary_set_loader = dataio_prep(params, meta_per_rec_file, split_type)
        ...
        out_rttm_dir = os.path.join(
            params["sys_rttm_dir"], params["mic_type_Eval"], split, tag
        )
...
def dev_pval_tuner(full_meta, split_type):
    ...
    """
    #print("I am inside dev_pval_tuner")
    DER_list = []
    #prange = np.arange(0.002, 0.03, 0.001) # CSC , GMM
    prange = np.arange(0.002, 0.003, 0.001)  # ASC, EER-Delta, SC-pNA  
...
def dataio_prep(hparams, json_file, split_type):
    ...
    #Defining the audio pipeline based on the split type
    if split_type == "dev":
    ...
    if split_type == "eval":
        # 2. Define audio pipeline.
        #print("We are here!")
        if params["mic_type_Eval"] == "Array1": 
            # Multi-mic (Microphone Array)
            @sb.utils.data_pipeline.takes("wav")
            @sb.utils.data_pipeline.provides("sig")
            def audio_pipeline(wav):
    ...
    if not params["skip_prep"]:
        run_on_main(
            ...
            kwargs={
                ...
                "mic_type_Eval": params["mic_type_Eval"], 
