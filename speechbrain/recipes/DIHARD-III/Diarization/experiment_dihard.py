...
from speechbrain.processing import diarization_dihard as diar #CHANGE FOR DIHARD3
...
#from transformers import Wav2Vec2FeatureExtractor, Wav2Vec2Processor, Wav2Vec2Model #added by subhajit
#from speechbrain.pretrained import EncoderClassifier # added by subhajit
...
# You may want to cache these globally to avoid reloading for every call
#ecapa_model = None
#titanet_model = None

# Functionto compute embeddings from the waveforms.
...
def embedding_computation_loop(split, set_loader, stat_file):
    ...
    if not os.path.isfile(stat_file):
        ...
        for batch in set_loader:
            ...
            ids = batch.id
            #print("The ids are:", ids) # It is the recordingID
            #file_paths = batch.path
            wavs, lens = batch.sig
            #print("The variable wavs is:", wavs) 
            #file_paths = batch.path
            #print("The wavs are:", wavs) # It stores the tensor of audio waveforms

            # Check if the audio length in samples is greater than 3200.
            if wavs.shape[1] > 700: # 500 was previous configuration
...
def diarize_dataset(full_meta, split_type, n_lambdas, pval, n_neighbors=10):
    ...
    for rec_id in tqdm(all_rec_ids):
        ...
        if params["oracle_n_spkrs"] is True:
            ...
            num_spkrs = diar.get_oracle_num_spkrs(rec_id, rttm)
        ...
        if params["backend"] == "SC_pNA":
            # Go for Self-Tuning Spectral Clustering (SC-pNA).
            diar.do_spec_clustering_SC_pNA(
                diary_obj,
                out_rttm_file,
                rec_id,
                num_spkrs,
...
def dev_pval_tuner(full_meta, split_type):
    ...
    DER_list = []
    #prange = np.arange(0.002, 0.030, 0.001) # CSC (SC)
...
def dataio_prep(hparams, split_type, json_file):
    ...
    @sb.utils.data_pipeline.provides("sig") # DEFAULT
    #@sb.utils.data_pipeline.provides("sig", "path")
    
    
    def audio_pipeline(wav):    #DEFAULT
        sig = read_audio(wav)
        return sig 
    ...
    # 3. Set output:
    sb.dataio.dataset.set_output_keys([dataset], ["id", "sig"]) # DEFAULT
    #sb.dataio.dataset.set_output_keys([dataset], ["id", "sig", "path"]) 
...
if __name__ == "__main__":  # noqa: C901
    ...
    if process_subset:
        ...
        #dev_list_path = data_folder_dev + 'docs/recordings.meeting.tbl' # I changed it to recordings.meeting.tbl from recordings.tbl
        df = pd.read_csv(dev_list_path, sep='\t')
        filtered_df = df[df.iloc[:, 3] == subset_id_dev]
        dev_list = filtered_df.iloc[:, 0].tolist()
    
        eval_list_path = data_folder_eval + 'docs/recordings.tbl' # I changed it to recordings.meeting.tbl from recordings.tbl
        #eval_list_path = data_folder_eval + 'docs/recordings.meeting.tbl' # I changed it to recordings.meeting.tbl from recordings.tbl
