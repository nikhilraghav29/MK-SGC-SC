...
from speechbrain.processing import voxconverse_diarization as diar #CHANGE FOR voxconverse
#import dihard3_diarization_SC_pNA as diar
...
def embedding_computation_loop(split, set_loader, stat_file):
    ...
    if not os.path.isfile(stat_file):
        ...
        #print("The length of setloader is:",len(set_loader))
...
def diarize_dataset(full_meta, split_type, n_lambdas, pval, n_neighbors=10):
    ...
    if params["oracle_n_spkrs"] is True:
        print("path to the reference directory",params["ref_rttm_dir"])
        full_ref_rttm_file = (
            params["ref_rttm_dir"] + "/fullref_voxconverse_" + split_type + ".rttm"
        )

        rttm = diar.read_rttm(full_ref_rttm_file)

        spkr_info = list(  # noqa F841
            filter(lambda x: x.startswith("SPEAKER"), rttm)
        )
        print("spkr_info is",spkr_info)

    # Get all the recording IDs in this dataset.
    all_keys = full_meta.keys()
    A = ['_'.join(line.split('_')[:1]) for line in all_keys]                #EROOR IS HERE
    all_rec_ids = list(set(A[1:]))
    all_rec_ids.sort()

    split = "voxconverse_" + split_type
    i = 1
    ...
    for rec_id in tqdm(all_rec_ids):
        ...
        #print("Shape of diary_obj is:",dir(diary_obj))
        ...
        if params["oracle_n_spkrs"] is True:
            ...
            #print("The rec id is:",rec_id)
            num_spkrs = diar.get_oracle_num_spkrs(rec_id, spkr_info)
            #print("The value of k_oracle is:", num_spkrs)
        else:
        ...
        if params["backend"] == "MeanShift":
            # call MeanShift
            diar.do_mean_shift_clustering(
                diary_obj, 
                out_rttm_file, 
                rec_id, 
                num_spkrs,
...
def dev_pval_tuner(full_meta, split_type):
    ...
    DER_list = []
    #prange = np.arange(0.002, 0.03, 0.001) # CSC (SC)
    prange = np.arange(0.002, 0.003, 0.001) # ASC, SC-pNA, EER-Delta
    ...
    for p_v in prange:
        ...
        ref_rttm = os.path.join(params["ref_rttm_dir"], "fullref_voxconverse_dev.rttm")
...
def dataio_prep(hparams, split_type, json_file):
    ...
    # Debug: Check dataset size and content
    ...
    def audio_pipeline(wav):
        sig = read_audio(wav)
        #print("shape of sig is:",sig.shape)
        return sig
...
def prepare_segs_for_RTTM_dev(
    ...
    for main_meet_id in list_ids:
        ...
        file_path = audio_dir + 'voxconverse-master/dev/' + main_meet_id + '.rttm'
...
def prepare_segs_for_RTTM_eval(
    list_ids, out_rttm_file, audio_dir, annot_dir, split_type
):
    ...
    for main_meet_id in list_ids:
        ...
        file_path = audio_dir + 'voxconverse-master/test/' + main_meet_id + '.rttm'
...
def prepare_metadata_dev(
    ...
    for row in SUBSEGMENTS:
        ...
        #print("wav_file_path:",wav_file_path,dur,start_sample,end_sample)
...
if __name__ == "__main__":  # noqa: C901
    ...
    if process_subset:
        ...
        eval_list = filtered_df.iloc[:, 0].tolist()
    """
    mypath_dev = "/home/nikhil/Documents/dataset/VoxConverse/voxconverse-master/dev/"
    dev_list = sorted([f[0:-5] for f in listdir(mypath_dev) if isfile(join(mypath_dev, f))])

    mypath_eval = "/home/nikhil/Documents/dataset/VoxConverse/voxconverse-master/test/"
    eval_list = sorted([f[0:-5] for f in listdir(mypath_eval) if isfile(join(mypath_eval, f))])    

    # Meta files
    meta_files = [
        os.path.join(meta_data_dir, "voxconverse_dev.subsegs.json"),
        os.path.join(meta_data_dir, "voxconverse_eval.subsegs.json"),
    ]
    ...
    opt_file = "opt_voxconverse_prepare.pkl"




    msg = "\tCreating meta-data file for the voxconverse Dataset.."
    logger.debug(msg)
    ...
    data_dev = "/home/nikhil/Documents/dataset/VoxConverse/"
    data_eval = "/home/nikhil/Documents/dataset/VoxConverse/"
    # Create reference RTTM files
    splits = ["dev", "eval"]
    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_voxconverse_" + i + ".rttm"
        if i == "dev":
            prepare_segs_for_RTTM_dev(
                dev_list,
                rttm_file,
                data_dev,
                manual_annot_folder,
    ...
    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_voxconverse_" + i + ".rttm"
        meta_filename_prefix = "voxconverse_" + i
        if i == "dev":
            prepare_metadata_dev(
                rttm_file,
                meta_data_dir,
                data_folder_dev,
    ...
    # voxconverse Dev Set: Tune hyperparams on dev set.
    ...
    best_nn = None
    if params["affinity"] == "nn":
        logger.info("Tuning for nn (Multiple iterations over voxconverse Dev set)")
        best_nn = dev_nn_tuner(full_meta, "dev")

    n_lambdas = None
    best_pval = None

# In the first if condition I removed the params["backend"] == "ASC" as we don't require any pval for that case.
    ...
    # Final print DERs
    msg = (
        "Final Diarization Error Rate (%%) on voxconverse corpus: Dev = %.2f %% | Eval = %.2f %%\n"
